...
from models.market.stocks import StockFinancial, StockPrice, StockQuery
from models.errors import Error


class GoogleFinanceClient(StockClient):
    id: str
    ...
    def fetch(self, query: StockQuery):
        data = None
        ...
        try:
            ...
            sheets_input = '=GOOGLEFINANCE("{0}", "{1}", {2}, {3}, {4})'.format(
                query.symbol,
                query.attr,
                query.start_date.strftime("DATE(%Y,%m,%d)"),
                query.end_date.strftime("DATE(%Y,%m,%d)"),
                query.interval,
            )

            input_value = {"values": [[sheets_input]]}
            ...
            for i, _ in enumerate(stock_data):
                ...
                for j, field in enumerate(StockPrice.__dataclass_fields__):
    ...
    def attr(self, query: StockQuery):
        data = None

        try:
            sheets_input = '=GOOGLEFINANCE("{0}", "{1}")'.format(
                query.symbol, query.attr
            )
            input_value = {"values": [[sheets_input]]}
    ...
    def attrs(self, query: StockQuery):
        data = None
        ...
        try:
            ...
            sheets_input = [
                '=GOOGLEFINANCE("{0}", "{1}")'.format(query.symbol, i)
                for i in attributes
            ]
...
finance = GoogleFinanceClient("1e3km5HHUvzEJx53kCkErvtldAr8_zrUgzXEvo757TYs")
